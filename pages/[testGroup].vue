...
cookieTest.value = {} as CookieTest

cookieTest.value.test_group = parseInt(`${route.params.testGroup}`)

function goToTest(e: Event) {
  cookieTest.value.gender = (
    (e.target as HTMLFormElement).elements.namedItem('gender') as HTMLSelectElement
  ).value
  cookieTest.value.age = parseInt(
    ((e.target as HTMLFormElement).elements.namedItem('age') as HTMLInputElement).value
  )
  cookieTest.value.user_agent = userAgent
  cookieTest.value.respondent_results = []

  navigateTo({ name: 'text-testGroup', params: { testGroup: cookieTest.value.test_group } })
}
</script>
